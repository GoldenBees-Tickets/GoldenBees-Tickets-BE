...
const { Payment, Cinema, Booking, Ticket, BookingSeat, Seat, User, Showtime, Movie, Room, Order, OrderCombo, PromotionUsage, sequelize } = require('../models');
...
const payWithMoMo = async (data) => {
    ...
    try {
        let { user_id, total, seat_ids, showtime_id, combos, promotion_id, orderInfo = 'Thanh toán vé xem phim' } = data;
        ...
        const ticketPromises = seat_ids.map(async (item) => {
            await Ticket.create({ order_id, seat_id: item.id, price: item.price });
        });

         // Tạo danh sách combo
        const orderComboPromises = combos.map(async (item) => {
            await OrderCombo.create({ order_id, combo_id: item.id, quantity: item.quantity });
        });
        
        await Promise.all([...ticketPromises], [...orderComboPromises]);
...
const generateTicketCode = (order_id) => {
    return 'TIX' + Date.now().toString().slice(-6) + Math.floor(Math.random() * 1000).toString().padStart(3, '0') + order_id;
};
...
const checkPaymentStatus = async (orderId) => {
    ...
    try {
        ...
        await transaction.commit();
            ...
            const movieData = await Movie.findOne({ where: { id: showtimeData.movie_id } });
