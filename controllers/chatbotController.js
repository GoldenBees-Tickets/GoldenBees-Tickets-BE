...
exports.generateResponse = async (req, res) => {
  try {
    ...
    if (Array.isArray(rooms.rooms) && rooms.rooms.length > 0) {
      ...
      for (let i = 0; i < Math.min(3, rooms.rooms.length); i++) {
        try {
          const room = rooms.rooms[i];
          ...
          // Thêm vào mảng thay vì ghi đè
          allSeats.push(...seatList);
        } catch (error) {
          console.error(
    ...
    const comboData = combos.map(combo => {
      // Lấy danh sách các item trong combo
      ...
      // Số tiền tiết kiệm
      const savings = totalIndividualPrice - combo.price;
      
      return {
        id: combo.id,
        name: combo.name,
        price: combo.price,
        regularPrice: totalIndividualPrice,
        savings: savings,
...
- Email hỗ trợ: support@movies-tickets.vn
  ...
  // Thêm thông tin về Combo bắp nước
  let comboInfo = "THÔNG TIN COMBO BẮP NƯỚC VÀ ĐỒ ĂN:\n";
  if (comboData && comboData.length > 0) {
    comboData.forEach(combo => {
      comboInfo += `
Combo: ${combo.name}
Giá: ${combo.price.toLocaleString('vi-VN')}đ
Giá gốc nếu mua lẻ: ${combo.regularPrice.toLocaleString('vi-VN')}đ
Tiết kiệm: ${combo.savings.toLocaleString('vi-VN')}đ (${combo.savingsPercent}%)
Bao gồm: ${combo.items.map(item => `${item.quantity} x ${item.name}`).join(', ')}
      `;
    });
  } else {
    comboInfo += "Hiện chưa có thông tin chi tiết về combo bắp nước.";
  }
...
Khi trả lời về combo bắp nước:
- Cung cấp thông tin chính xác về các combo có sẵn
- Giải thích rõ ràng về giá tiền, tiết kiệm được bao nhiêu khi mua combo
- Có thể đề xuất combo phù hợp dựa trên số người hoặc nhu cầu của khách hàng
- Trả lời câu hỏi về thành phần trong combo một cách chi tiết
